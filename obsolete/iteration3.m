function [T] = iteration3(Tx,M,iinvA,ite)
    ...
    for k = 1:ite
        ...
        for i=1:numRows-1
            for j = i+1 : numRows
                tAiT = iinvA(1:3,4,i)';
                RAiT = iinvA(1:3,1:3,i)';
                tAjT = iinvA(1:3,4,j)';
                RAjT = iinvA(1:3,1:3,j)';
                piT = M(i,1:3)';
                pjT = M(j,1:3)';
                di = M(i,4);
                dj = M(j,4);
                calc_g = RAiT*RxT*piT-RAjT*RxT*pjT;
                g = [g,calc_g'];
                calc_J = -RAiT*skew(RxT*piT)+RAjT*skew(RxT*pjT);
                J = [J,calc_J'];
            end
        end
        std(g)
        ...
        J = J';
        delta_x = -inv(J'*J)*J'*g
        RxT = lie_to_rotate(delta_x(1:3,1)*0.1)*RxT
    end
    T = [RxT',txT';0,0,0,1];
end
